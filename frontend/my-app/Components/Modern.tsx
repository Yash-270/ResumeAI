...
export default function Modern({ data }: { data: ResumeData }) {
  ...
  <div className=" flex flex-col items-center justify-center px-6 py-10 bg-[#0a0f1c] text-white relative overflow-hidden">
    ...
    <p className="text-sm text-blue-500 mt-1">
      ...
      {data?.summary && (
        <p className="mt-3 text-gray-700 text-sm leading-relaxed border-l-4 border-blue-500 pl-3">{data.summary}</p>
      )}

      {data?.skills?.length>0 && (
        <div className="mt-5">
          <h2 className="font-bold text-blue-600 uppercase text-xs tracking-widest border-b border-gray-300 pb-1 mb-3">Skills</h2>
          <div className="flex flex-wrap gap-2">
            {safeArray(data.skills).map((s, i) => (
              <span key={i} className="bg-blue-50 border border-blue-200 text-blue-800 px-3 py-1 rounded-full text-xs">{renderItem(s)}</span>
            ))}
          </div>
        </div>
      )}
      ...
      {data?.projects?.length>0 && (
        <div className="mt-5">
        <h2 className="font-bold text-blue-600 uppercase text-xs tracking-widest border-b border-gray-300 pb-1 mb-3">Projects</h2>
        <ul className="mt-2 space-y-1">
          {safeArray(data.projects).filter(item => item && item !== "").map((p, i) => <li key={i} className="text-sm text-gray-700">▸ {renderItem(p)}</li>)}
        </ul>
        </div>
      )}
      ...
      {data?.experience?.length>0 && (
        <div className="mt-5">
          <h2 className="font-bold text-blue-600 uppercase text-xs tracking-widest border-b border-gray-300 pb-1 mb-3">Experience</h2>
          <ul className="mt-2 space-y-1">
            {safeArray(data.experience).filter(item => item && item !== "").map((e, i) => <li key={i} className="text-sm text-gray-700">▸ {renderItem(e)}</li>)}
          </ul>
        </div>
      )}
      ...
      {data?.education?.length>0 && (
        <div className="mt-5">
          <h2 className="font-bold text-blue-600 uppercase text-xs tracking-widest border-b border-gray-300 pb-1 mb-3">Education</h2>
          <ul className="mt-2 space-y-1">
            {safeArray(data.education).filter(item => item && item !== "").map((ed, i) => <li key={i} className="text-sm text-gray-700">▸ {renderItem(ed)}</li>)}
          </ul>
        </div>
      )}
      <div

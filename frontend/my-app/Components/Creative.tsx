...
export default function Creative({ data }: { data: ResumeData }) {
  ...
  <div className=" flex flex-col items-center justify-center px-6 py-10 bg-[#0a0f1c] text-white relative overflow-hidden">
      ...
      {/* Header - centered with gradient */}
      <div className="text-center mb-6 pb-4 border-b-2 border-gray-200">
        {data?.name && (
          <h1 className="text-4xl font-bold bg-gradient-to-r from-purple-600 to-blue-500 bg-clip-text text-transparent">{data.name}</h1>
        )}
        <p className="text-sm text-gray-500 mt-2 flex justify-center flex-wrap gap-3">
          {data?.email && <span>📧 {data.email}</span>}
          {data?.phone && <span>📞 {data.phone}</span>}
        </p>
        <p className="text-sm text-purple-500 mt-1 flex justify-center flex-wrap gap-3">
          {data?.linkedin && <span>🔗 {data.linkedin}</span>}
          {data?.github && <span>🐙 {data.github}</span>}
        </p>
      </div>
      ...
      {data?.summary && (
        <p className="mt-3 text-gray-700 text-sm leading-relaxed border-l-4 border-purple-500 pl-3 bg-purple-50 py-2 rounded-r-lg">{data.summary}</p>
      )}

      {data?.skills?.length>0 && (
        <div className="mt-5">
          <h2 className="font-bold text-purple-600 uppercase text-xs tracking-widest border-b-2 border-purple-200 pb-1 mb-3">Skills</h2>
          <div className="flex flex-wrap gap-2">
            {safeArray(data.skills).map((s, i) => (
              <span key={i} className="bg-gradient-to-r from-purple-50 to-blue-50 border border-purple-200 text-purple-800 px-3 py-1 rounded-full text-xs font-medium">{renderItem(s)}</span>
            ))}
          </div>
        </div>
      )}
      ...
      {data?.projects?.length>0 && (
        <div className="mt-5">
          <h2 className="font-bold text-blue-600 uppercase text-xs tracking-widest border-b-2 border-blue-200 pb-1 mb-3">Projects</h2>
          <ul className="mt-2 space-y-2">
            {safeArray(data.projects).filter(item => item && item !== "").map((p, i) => (
              <li key={i} className="text-sm text-gray-700 flex gap-2">
                <span className="text-purple-500 font-bold">▸</span>{renderItem(p)}
              </li>
            ))}
          </ul>
        </div>
      ...
      {data?.experience?.length>0 && (
        <div className="mt-5">
          <h2 className="font-bold text-blue-600 uppercase text-xs tracking-widest border-b-2 border-blue-200 pb-1 mb-3">Experience</h2>
          <ul className="mt-2 space-y-2">
            {safeArray(data.experience).filter(item => item && item !== "").map((e, i) => (
              <li key={i} className="text-sm text-gray-700 flex gap-2">
                <span className="text-purple-500 font-bold">▸</span>{renderItem(e)}
              </li>
            ))}
          </ul>
        </div>
      ...
      {data?.education?.length>0 && (
        <div className="mt-5">
          <h2 className="font-bold text-blue-600 uppercase text-xs tracking-widest border-b-2 border-blue-200 pb-1 mb-3">Education</h2>
          <ul className="mt-2 space-y-2">
            {safeArray(data.education).filter(item => item && item !== "").map((ed, i) => (
              <li key={i} className="text-sm text-gray-700 flex gap-2">
                <span className="text-purple-500 font-bold">▸</span>{renderItem(ed)}
              </li>
            ))}
          </ul>
        </div>
